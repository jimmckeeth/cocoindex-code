...
def main():
    ...
    parser.add_argument("--clear-cache", action="store_true", help="Ignore cache and download fresh data.")
    ...
    # --- TASK DEFINITIONS ---
    retrieval_tasks = ['MSMARCO', 'NQ', 'HotpotQA', 'FiQA2018', 'ArguAna', 'Touche2020', 'SCIDOCS', 'SciFact', 'NFCorpus', 'QuoraRetrieval', 'DBPedia']
    code_tasks = ['CodeSearchNetRetrieval', 'CosQA', 'StackOverflowQA', 'HumanEvalRetrieval', 'MBPPRetrieval']
    ...
    results = model_scores.merge(code_scores, on='model_name', how='outer', suffixes=('_general', '_code'))
    
    # Only show public models
    public_models = df[df['is_public'] == True]['model_name'].unique()
    ...
    def categorize(size):
        ...
        if size < 50: return "Micro (< 50M)"
        if size < 150: return "Small (< 150M)"
        if size < 500: return "Medium (< 500M)"
        return "Large (> 500M)"
    ...
    for tier in tiers:
        ...
        tier_df = results[results['tier'] == tier].sort_values(by='score_code', ascending=False).head(5)
        if tier_df.empty:
    ...
    report.append("This report was generated using the `find_best_models.py` script. To update it with the latest live data from MTEB, run:")
    report.append("```bash")
